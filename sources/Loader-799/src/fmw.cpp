...
uint32_t countProgramCRC()
{
 uint8_t *p;
 uint32_t crc, sz;
 p = (uint8_t*)MAIN_PROG_FIRST_ADDR;  // адрес с которого начинается основная программа  //-V566
 sz = *((uint32_t*)(MAIN_PROG_FIRST_ADDR+DATA_BLOCK_OFFSET+2*4)); // размер программы  //-V566
 if( sz > 0xFFFFFUL )
   {
...
uint8_t CheckMainProgControlSum( void )
{
 ...
 volatile uint32_t fcrc = *((uint32_t*)(MAIN_PROG_FIRST_ADDR+512+3*4) ); //  сохраненная контрольная сумма  //-V566
...
int check_file()
{
 ...
 while( len>0 )
   {
       HAL_SPI::ReadBuffer( DirectionSPI::Memory, addr, buffer, STEP ); // читаем
    if( a >len )
      a= len;  
   ...
   HAL_SPI::ReadBuffer(DirectionSPI::Memory, UPDATE_START_ADDR+file_size, buffer, 8 ); // читаем контрольную сумму
 if( NTOH( buffer ) == crc )
   return 1;  
 else 
   return 0;  
...
{ // читаем заголовок файла
    HAL_SPI::ReadBuffer(DirectionSPI::Memory, UPDATE_START_ADDR, fileHeader, 8*4 ); // читаем заголовок файла
}
