...
uint Math::CalculateCRC32(const void *buffer, int size)
{
    return ProcessCRC((const uint8 *)buffer, size, 0);
}


uint Math::ProcessCRC(const uint8 *data, int length, uint crc)
{
    while (length--)
    {
...
bool Math::ArraysEquals(uint8 *array1, uint8 *array2, uint8 *array3, int size)
{
