...
{
    SwitchTemperatureIndicator::Update();

    if (ModeReader::Current() == ModeReader::WG &&
        ROMWriter::IsCompleted() &&
        ModeIndicator::IsExternal() &&
        Device::IsInitializationCompleted() &&
        !ModeOffline::IsEnabled() &&
        !OSDP::IsEnabled() &&
        !HAL::Is765())
    {
        ...
        if (lg != prev_lg || lr != prev_lr ||
            (prev_state_low_energy && !Power::IsLessThanNormal()))    // Если вышли из режима пониженного питания
        {
            if (lg && lr)
            {
                if (!lg_and_lr)
        ...
        prev_state_low_energy = Power::IsLessThanNormal();
    }
